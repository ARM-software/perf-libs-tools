...
def logFFT(readline, fftNames, fftLens, fftCnts, fftHowmany, fftTimes, fftPlanPtrs, fftExecPtrs):
  ...
  if (routine.endswith("fft1mx_")) :
    plan = int(readline[7])
    nFFTs = int(readline[9])
    fftLen = [int(readline[10])]
  elif (routine.endswith("fft1m_")) :
    plan = int(readline[7])
    nFFTs = int(readline[8])
    fftLen = [int(readline[9])]
    ...
    plan = int(readline[7])
    nFFTs = int(1)
    fftLen = [int(readline[12]),int(readline[10]),int(readline[11])]
    ...
    return
  elif (routine=="fftw_destroy_plan" or routine=="fftwf_destroy_plan" or routine=="dfftw_destroy_plan" or routine=="sfftw_destroy_plan") :
    return
  else :
...
def logBLAS(readline, blasNames, blasCnts, blasTimes):
...
def logLAPACK(readline, lapackNames, lapackCnts, lapackTimes):
...
# open input/output files
  ...
  for position in range (1, nArguments+1) :
    ...
    for line in  inputfile:
      ...
      if (category==0 or category==1 or category==2) :
        logBLAS(readline, blasNames[category], blasCnts[category], blasTimes[category])

      # LAPACK : Whatever's left!
      if (category == -1 or category==3) :
        logLAPACK(readline, lapackNames, lapackCnts, lapackTimes)
        category=3
  ...
  print "BLAS level 1     : count %10s    total time %12.4f" % (cnt[0], tottime[0]) 
  print "BLAS level 2     : count %10s    total time %12.4f" % (cnt[1], tottime[1]) 
  print "BLAS level 3     : count %10s    total time %12.4f" % (cnt[2], tottime[2]) 
  print "LAPACK           : count %10s    total time %12.4f" % (cnt[3], tottime[3]) 
  print "FFT              : count %10s    total time %12.4f" % (cnt[4], tottime[4]) 
  print " "
  print "Double precision : count %10s    total time %12.4f" % (datatype_cnt[0], datatype_time[0])
  print "Single precision : count %10s    total time %12.4f" % (datatype_cnt[1], datatype_time[1])
  print "Double complex   : count %10s    total time %12.4f" % (datatype_cnt[2], datatype_time[2])
  print "Single complex   : count %10s    total time %12.4f" % (datatype_cnt[3], datatype_time[3])
  ...
  if (len(blasNames[0])+len(blasNames[1])+len(blasNames[2])>0) :
    ...
    for category in range(0,3) :
       if (len(blasNames[category])>0) :
          ...
          # convert to a tuple for sorting
          unorderedTuple = zip(blasNames[category], blasCnts[category], blasTimes[category])
          orderedTuple = sorted(unorderedTuple, key=lambda field: field[2], reverse=True)
          for i in range(0,len(blasNames[category])) :
             print "%8s     cnt= %10d  totTime= %12.4f " % (orderedTuple[i][0], orderedTuple[i][1], orderedTuple[i][2])
  ...
  if (len(lapackNames)>0) :
    ...
    # convert to a tuple for sorting
    unorderedTuple = zip(lapackNames, lapackCnts, lapackTimes)
    orderedTuple = sorted(unorderedTuple, key=lambda field: field[2], reverse=True)
